...
// --- 简单必填校验 ---
function validate(fields, isSubscribe) {
  const errors = [];
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const isSubscribe = (raw.Form_Kind || '').trim() === 'subscribe';
    ...
    const fields = {
      ...
      lang: (raw.lang || raw.Form_Language || 'en').trim(),
    };

    // 2. 校验（订阅只校验 email；普通询价校验 name/email/message）
    const missing = validate(fields, isSubscribe);
    if (missing.length > 0) {
      return jsonResponse(
    ...
    const subject = isSubscribe
      ? `[Kalistorik Subscribe] ${safe.email}`
      : `[Kalistorik Contact] ${safe.name} — ${safe.product || 'general'}`;

    const textBody = isSubscribe
      ? `
...
`.trim();
    ...
    const htmlBody = isSubscribe
      ? `
<!doctype html>
<html>
  <body style="font-family: -apple-system, system-ui, sans-serif; color:#222; line-height:1.5;">
    <h2 style="color:#C8102E; margin-bottom:0;">New Newsletter Subscription</h2>
