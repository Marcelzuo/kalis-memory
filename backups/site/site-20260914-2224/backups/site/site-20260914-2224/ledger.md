...
## 2026-09-14 · 米迦勒 · 独立站 SEO + AI 可发现性优化

event=done
task=传统 SEO 与 AI/LLM 可发现性一起优化
commit=cacbd5c · feat: add ai search optimization and seo schema
deploy=GitHub Actions success

### 已完成
- 技术 SEO：`robots.txt` 移除本地 Google-Extended 禁止；`sitemap.xml` 18 个 URL 全量补 `lastmod`。
- 首页：双 H1 合并为 1 个可见 H1；补 `FAQPage` JSON-LD（6 组 EN 问答）。
- 博客：5 篇文章补 `Article` JSON-LD、author、twitter card、og image；博客 index 补 Organization + WebSite JSON-LD。
- AI 可读层：重写 `llms.txt`，新增 `llms-full.txt`，新建 `/okf/`（index/company/services/faq/blog-index）。
- 上线验证：`/llms.txt`、`/llms-full.txt`、`/okf/index.md`、博客 schema、首页 FAQPage 均返回 200。

### 已解决（2026-09-14 追加）
- 复用本机已登录的 Cloudflare 控制台会话，在 `security/settings?tabs=bot-traffic` 关闭「阻止 AI 自动程序」并将「管理您的 robots.txt」改为「禁用 robots.txt 配置」。
- Cloudflare 状态：`ai_bots_protection=disabled`、`is_robots_txt_managed=false`。
- 线上复测：GPTBot / ClaudeBot / PerplexityBot / meta-externalagent / Bytespider / Googlebot 首页均 200；`robots.txt` 为本地 `Allow: /` + sitemap。

---

## 2026-09-14 · 米迦勒 · 独立站焕新 + 审计修复 + 部署 + 工作文件归档

event=done
task=独立站板块焕新、全站三兄弟独立审核、Favicon 统一、修复审计问题、部署上线、归档临时工作文件
backup=/Users/zuo/Marcelzuo/kalis-memory/backups/site/site-20260914-2053 · commit b0df4a9
deploy=/Users/zuo/kalistorik-site · commit dd56131 · GitHub Actions `Deploy to CF Pages` success

### 本轮已完成
- 首页/二级页内容焕新：Our Picks、Why Us、How It Works、Why our clients say、二级页展示内容已按本轮要求更新；产品顺序按要求调整，Sofa 作为首页首屏主品类。
- 博客板块已建立，直接承接 LinkedIn 内容源；上线 4 篇博客页。
- Favicon 统一：以当前黑色 K `favicon.svg` 配套 PNG/ICO 为唯一标准。
- 修复：联系表单字段契约、`resp.success` 读取、Email 联系入口、次级页移动端导航、博客/次级页页脚社媒链接、404/wa 回退、GA4 接入、SEO URL/canonical/og/hreflang/sitemap/llms.txt/webmanifest。
- 线上验证通过：`/en`、`/blog`、`/picks.html` 等路径 308 跳转正常；抽查 canonical/favicon/GA4 正常。

### 工作文件归档
- 旧版设计稿/旧博客源稿/旧 content-refresh 临时稿已同步到共享备份库 `work/kalistorik-site/20260914/`。
- 主站本地已清理 `_work-archive/`、`blog/source/`、`content-refresh/`，仓库保持干净；这些文件不再进入 Cloudflare Pages 部署范围。
- 三个审计产物保留在 `/tmp/kt-audit/`（不在 git 仓库内）。

### 下一步待办
- 已统一归档到共享备份库，待办关闭。
- 下一次 SEO 专项再做，本轮不提前动技术 SEO 新项。
